...
export function useCascadingSelect(args: {
  ...
  const [val, setVal] = useState<{ options: any[] }[]>(() => {
    ...
    _.times(args.columns.length - 1, () =>
      defaultResponse.push({ options: [] })
    );
  ...
  const queryState = api.useInfiniteQueryMany({
    where,
    pageSize: args.pageSize ?? 500,
    distinctOn: args.columns[selected.length],
  });
  ...
  const select = (columnIndex: number, selectedValue: string) => {
    ...
    const newWhere = newSelected.reduce(
      (where: any, selected: string, idx: number) => {
