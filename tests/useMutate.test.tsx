...
describe("useInsert", () => {
  ...
  it("inserts with updating item values", async () => {
    const wrapper = wrapperWithResultValue(
      { id: "456", groupId: "123", postId: "897" },
      "mutation"
    );
...
describe("useDelete", () => {
  ...
  it("correctly changes item and variables when calling executeMutation", async () => {
    ...
    expect(result.current.mutationConfig.variables.userId).toEqual("test");
    expect(
      (result.current.mutationConfig.variables.item as JsonObject)?.["postId"]
    ).toEqual("456");
    ...
    expect(result.current.mutationConfig.variables.testVariable).toEqual(
      "testVariable"
    );
  });
});
